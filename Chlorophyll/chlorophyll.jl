...
function chlorophyll_graph!()::Nothing
    data = CSV.read("Chlorophyll/cmems_mod_glo_bgc-pft_anfc_0.25deg_P1D-m_1720520576148.csv", DataFrame, header=8)
    ...
    plin = lines!(-zc, zdata)
    xlims!(0, 100)
    Legend(fig[1, 2], [clin, plin], ["Chlorophyll from MyOcean", "Scaled phytoplankton from model"])
    display(fig)
    return nothing
end